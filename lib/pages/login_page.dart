...
class _LoginPageState extends State<LoginPage> {
  ...
  /// 打开学校统一身份认证（内嵌浏览器）。滑块与短信验证由用户自己完成，
  /// 之后在大厅里打开课表并点「导入课表」，数据结构与原来完全一致。
  Future<void> _openWebLogin() async {
    var imported = false;
    await Navigator.of(context).push(MaterialPageRoute(
      builder: (_) => WebLoginPage(onImported: (_) => imported = true),
    ));
    if (!mounted) return;
    if (imported) widget.onDone();
  }
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.stretch,
                      children: [
                        const Text('推荐：学校统一身份认证登录',
                            style: TextStyle(
                                fontSize: 13.5,
                                fontWeight: FontWeight.w700,
                                color: AppColors.primaryDark)),
                        const SizedBox(height: 4),
                        const Text('登录后在大厅里打开课表，点「导入课表」即可；'
                            '滑块与短信验证由你自己完成',
                            style: TextStyle(
                                fontSize: 12,
                                height: 1.4,
